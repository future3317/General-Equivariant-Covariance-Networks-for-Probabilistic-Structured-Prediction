...
| Matched dielectric law evidence freeze | `results/stat_misspec_matched_full_t_20260821/dielectric/{headline_metrics.json,submission_evidence_manifest.json}` | Same formal Full-t checkpoint per seed across fixed-$\nu$, global-$\nu$, conditional-scale, and conditional-$\nu(x)$; FP64 evaluator recomputes NLL/calibration and records Energy/radial/directional diagnostics |
...
|---|---|---|---|---|
| Matched dielectric conditional-nu | Conditional radial law improves the headline Full-t checkpoints, not only a separate frozen checkpoint | Same seed, features, mean, scatter, 281 test IDs; validation-only conditional-$\nu$ fit; paired law-correct metrics | Completed positive mechanism diagnostic: conditional-$\nu(x)$ NLL $-2.656\pm0.214$ versus conditional-scale $-2.410\pm0.134$; radial KS $0.116$ versus $0.236$; directional $p=0.005$ in all arms | Report tail-shape contribution beyond conditional scale; retain directional failure as a limitation |
| Matched dielectric conditional-scale | Sample-dependent invariant scale is a capacity-matched alternative explanation for the conditional-$\nu(x)$ effect | Same frozen E1 cache, invariant $0e$ readout, $S'(x)=\exp(\delta(x))S(x)$, validation-only selection | Completed three-seed negative control; FP64 evidence freeze and artifact manifest complete | Keep as the matched capacity control; do not promote scale as the primary mechanism |
...
| Random topology distribution | True skeleton is better than most degree-matched random trees | 12 Prüfer-sampled labeled degree-sequence trees, no outcome filtering, fixed seed 42, shared protocol, paired Side/Top NLL | Completed: all 12 null-minus-true effects positive; Side mean $14.791\pm2.976$ (min $8.557$), Top mean $23.339\pm4.795$ (min $15.213); all null predictions finite and strict-SPD | Report as a topology-specific likelihood effect; do not generalize to OOD calibration or Energy/MACE improvement |
