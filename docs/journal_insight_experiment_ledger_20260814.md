...
| Representation-compatible elasticity Full-t | `results/elasticity_stability_20260819/asinh_exp_formal_20260819/seed_{42,43,44}` on the server | The asinh full-image spectral chart is a semantics-compatible candidate distinct from the legacy Voigt stress test. Seeds 42 and 44 have complete artifacts; seed 43 is running under the fixed 12-epoch, validation-only protocol. No manuscript claim is eligible until all three seeds have complete artifacts and pass finite/FP64-SPD/equivariance gates. |
