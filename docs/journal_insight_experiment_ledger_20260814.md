...
| Matched dielectric conditional-scale | Sample-dependent invariant scale is a capacity-matched alternative explanation for the conditional-$\nu(x)$ effect | Same frozen E1 cache, invariant $0e$ readout, $S'(x)=\exp(\delta(x))S(x)$, validation-only selection | Contract implemented; formal server artifacts pending | Run only after a free GPU is available; promote the mechanism wording only from complete artifacts |
...
| Random topology distribution | True skeleton is better than most degree-matched random trees | Prüfer-sampled labeled degree-sequence trees, no outcome filtering, fixed split and protocol | Manifest regenerated; formal runs pending | Run the preregistered manifest only on an available GPU and audit all completed entries |
